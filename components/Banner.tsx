    ...
    <div className="flex flex-col space-y-2 py-16 md:space-y-4 lg:h-[65vh] lg:justify-end lg:pb-12">
        <div className="absolute top-0 left-0 -z-10 h-[95vh] w-screen">
            <Image 
              ...
              layout="fill"
              objectFit="cover"
              alt={bannerMovie?.title || bannerMovie?.name || "Banner Image"}
            />
        </div>
        <h1 className="text-2xl font-bold md:text-4xl lg:text-7xl">{bannerMovie?.title || bannerMovie?.name || bannerMovie?.original_name}</h1>
        <p className="max-w-xs  text-shadow-md text-xs md:max-w-lg md:text-lg lg:max-w-2xl lg:font-2xl">{bannerMovie?.overview}</p>
